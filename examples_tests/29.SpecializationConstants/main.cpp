...
//benchmark settings
//#define useImageStoreInsteadDrawMeshBuffer
...
int main()
{
	...
	{
#ifdef useImageStoreInsteadDrawMeshBuffer
		auto file = filesystem->createAndOpenFile("../particlesImageStore.comp");
#else
		auto file =filesystem->createAndOpenFile("../particles.comp");
#endif // useImageStoreInsteadDrawMeshBuffer
	...
	constexpr uint32_t PARTICLE_COUNT = 1u << 24;
	constexpr uint32_t PARTICLE_COUNT_PER_AXIS = 1u << 8;
...
#ifdef useImageStoreInsteadDrawMeshBuffer
	auto imageStoreUBO = driver->createDeviceLocalGPUBufferOnDedMem(sizeof(float) * PARTICLE_COUNT);;
	{
		video::IGPUDescriptorSet::SDescriptorInfo i[4];
		...
		w[2].binding = 2u;
		w[2].count = 1u;
		w[2].descriptorType = asset::EDT_STORAGE_IMAGE;
		w[2].dstSet = gpuds0Compute.get();
		w[2].info = i + 3;
		i[3].buffer.offset = 0;
		i[3].buffer.size = sizeof(float) * PARTICLE_COUNT;
		i[3].desc = imageStoreUBO;
	...
	uint32_t testCaseIndex = 0;
	while (device->run() && (testCaseIndex++) < TEST_CASE_COUNT)
	{
		auto* query = driver->createElapsedTimeQuery();
...
#ifndef useImageStoreInsteadDrawMeshBuffer
		...
		query->drop();
		os::Printer::print(std::string("Result ") + std::to_string(testCaseIndex) + std::string(": ") + std::to_string(static_cast<double>(timeElapsed) / 1000000.0) + std::string("ms."));
